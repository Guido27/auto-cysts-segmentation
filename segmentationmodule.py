...
class SegmentCyst(pl.LightningModule):
    def __init__(self, **hparams):
        ...
        self.weight = torch.tensor([0.1, 10.0]) # class with same weights in this datasets
    ...
    def training_step(self, batch, batch_idx):
        ...
        for rate in size_rates:
            ...
            patches, labels = unfold_patches(gts,images, size = self.p_size, stride = self.p_size)
            ...
            # refine predictions
            refined_predictions = refine_predictions_unfolding(logits, predicted_labels, size = self.p_size, stride = self.p_size)
    ...
    def validation_step(self, batch, batch_id):
        ...
        for rate in size_rates:
            ...
            patches, labels = unfold_patches(gts,images, size=self.p_size, stride=self.p_size) # here labels are used only to compute validation loss
    ...
    def test_step(self, batch, batch_id):
        ...
        for rate in size_rates:
            ...
            patches = unfold_patches(gts,images, test=True, size=self.p_size, stride = self.p_size) # here labels are used only to compute validation loss
